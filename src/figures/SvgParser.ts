// Script adapted from https://github.com/Yqnn/svg-path-editor/blob/master/src/app/svg-parser.ts
...
type PathArr = Array<string | number>;
...
export default class SvgParser {
  static components(type: string, path: string, cursor: number, size: number): [number, PathArr] {
    ...
    const components: PathArr = [];
    while (cursor <= path.length) {
      const component: PathArr = [type];
      for (const regex of expectedRegexList) {
        const match = path.slice(cursor).match(regex);

        if (match !== null) {
          component.push(Number(match[0]) / size);
  ...
  public static parse(path: string, size: number): PathArr {
    let cursor = 0;
    let tokens: PathArr = [];
    while (cursor < path.length) {
      ...
      if (match !== null) {
        ...
        cursor += match[0].length;
        const componentList = SvgParser.components(command, path, cursor, size);
